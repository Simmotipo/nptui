    ...
    class NPTUI
    {
        ...
        public static bool Load(string path)
        {
                ...
                for (int i = Utils.GetLineNumber(lines, "ethernets") + 1; i < lines.Length; i++)
                {
                    ...
                    else if (lines[i] != "" && Utils.GetIndentationLevel(lines[i], indent_count) == 2)
                    {
                        ...
                        else
                        {
                            ...
                            if (!newEthernet.name.ToLower().Contains("veth") && !newEthernet.name.ToLower().Contains("br-") && !newEthernet.name.ToLower().Contains("docker")) ethernets.Add(new Ethernet(workingLines));
                            workingLines = "";
            ...
            foreach (NetworkInterface ni in NetworkInterface.GetAllNetworkInterfaces())
            {
                ...
                if (!ni.Name.ToLower().Contains("veth") && !ni.Name.ToLower().Contains("br-") && !ni.Name.ToLower().Contains("docker")) 
