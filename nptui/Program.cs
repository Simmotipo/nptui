...
namespace NPTUI
{
    class NPTUI
    {
        public static string nptui_version = "v3.0";
            ...
            if (netplanPath != "")
            {
                ...
                if (File.Exists(netplanPath))
                {
                    try
                    {
                        ...
                        if (!Save(ethernets.ToArray(), bonds.ToArray(), netplanPath, previewOnly: false))
                        {
                            Console.WriteLine("An error occurred creating the file. Try sudo?");
                            Environment.Exit(0);
                        }
        ...
        {
            int selected_item = 0;
            string[] menu_options = ["Edit Interfaces", "Edit Bonds", "Edit Hostname", "About NPTUI", "Run Netplan Apply", "Exit"];
                    ...
                    case ConsoleKey.Spacebar:
                        switch (selected_item)
                        {
                            ...
                            case 4:
                                ...
                                Save(ethernets.ToArray(), bonds.ToArray(), netplanPath, previewOnly: true);
                                Console.WriteLine("\n   ".PadRight(64, '=') + "\n");
                ...
                for (int i = 0; i < menu_options.Length; i++)
                {
                    ...
                    foreach (Ethernet ethernet in ethernets)
                    {
                        if (ethernet.name == menu_options[i])
                        {
                            ...
                            {
                                if (bond.interfaceMacs.Contains(ethernet.macaddress)) { Console.ForegroundColor = ConsoleColor.DarkBlue; break; }
                            }
                            break;
                        }
                    }
            ...
            while (true)
            {
                ...
                switch (Console.ReadKey().Key)
                {
                    ...
                    case ConsoleKey.Spacebar:
                        ...
                        {
                            foreach (Bond b in bonds) if (b.name == menu_options[selected_item]) { EditBond(b); break; }
                            refreshMenuOptions = true;
                        }
                        break;
                }
        ...
        public static void EditInterface(Ethernet e, bool isBonded = false) // isBonded will disable IP config (this should be used if the interface is EITHER bonded, or has a vlan subinterface.)
        {
            ...
            while (true)
            {
                ...
                switch (Console.ReadKey().Key)
                {
                    ...
                    case ConsoleKey.Enter:
                    case ConsoleKey.Spacebar:
                        if (selected_item == menu_options.Length - 1) { Save(ethernets.ToArray(), bonds.ToArray(), netplanPath, previewOnly: false); return; }
        ...
        public static void EditBond(Bond b, bool isBonded = false) // isBonded will disable IP config (this should be used if the interface is EITHER bonded, or has a vlan subinterface.)
        {
            ...
            while (true)
            {
                if (refreshMenuOptions)
                {
                    ...
                    menuOptionsList.Add($"IPv4 Routes          | {route_count} custom route(s)".PadRight(64));
                    menuOptionsList.Add("");
                    menuOptionsList.Add("- Delete Bond");
                    menuOptionsList.Add("< Back To Menu");
                    menu_options = menuOptionsList.ToArray();
                    refreshMenuOptions = false;
                }
                ...
                switch (Console.ReadKey().Key)
                {
                    ...
                    case ConsoleKey.Spacebar:
                        ...
                        {
                            string ethernetNameToAdd = InterfaceSelectMenu(false);
        ...
        public static string InterfaceSelectMenu(bool includedBonded = true)
        {
            int selected_item = 0;
            bool refreshMenuOptions = true;
            string[] menu_options = [];
            while (true)
            {
                if (refreshMenuOptions)
                {
                    ...
                    foreach (Ethernet ethernet in ethernets)
                    {
                        if (includedBonded) { menuOptionsList.Add(ethernet.name);  continue; }
                        bool can_add = true;
        ...
        public static bool Load(string path)
        {
            ...
            foreach (Ethernet ethernet in ethernets)
            {
                foreach (Bond bond in bonds)
                {
                    ...
                    if (bond.name == ethernet.name) { ethernetsToDelete.Add(ethernet); break; }
                }
            }
        ...
        public static bool Save(Ethernet[] ethernets, Bond[] bonds, string netplanPath, int indent = 2, bool previewOnly = true)
        {
            string tab = "";
            ...
            if (ethernets.Length > 0)
            {
                ...
                foreach (Ethernet e in ethernets) finished_product += e.ToYaml(bonds:bonds) + "\n";
            }
            if (bonds.Length > 0)
            {
                finished_product += $"{tab}bonds:\n";
                foreach (Bond b in bonds) finished_product += b.ToYaml(ethernets:ethernets) + "\n";
            }
